...
int main(){
  ...
  LOOP(cn, tc){
    ...
    LOOP(i, m){
      cin >> x >> y >> c;
      el.push_back(tuple<int, int, int>(x-1, y-1,c));
    }
    ...
    cost = 0;
    for(auto edge : el){
      // Build a road if and only if the cost is smaller than the cost of building airport
      if( !isSameSet(get<0>(edge),get<1>(edge)) && get<2>(edge) < a){
        cost+= get<2>(edge);
        unionSet(get<0>(edge), get<1>(edge));
      }
    }

    // Build an airport for every disconnected components
    airports = totalSet;


    printf("Case #%d: %ld %ld\n", cn+1, cost+airports*a, airports);
  }
