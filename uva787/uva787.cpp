...
int main(){
  ...
  int temp;
  while(cin >> temp){
    v.push_back(BigInt(temp));
    vector<int> segments;
    while(true){
      cin >> temp;
      if (temp==0){
        segments.push_back(v.size()-1);
    ...
    for (int j = 0; j <= v.size(); j++) {
      for(int k = j; k <= v.size(); k++) {
        bool flag = false;
        for (int z : segments) {  // multiplied by 0
          if (j<= z && k >= z) {flag = true; break;}
        }
        if (flag){
          maxproduct = maxproduct > 0 ? maxproduct : 0;
        }
        else if (j > 0 && v[j-1] != 0){
          maxproduct = maxproduct > v[k]/v[j-1] ? maxproduct : v[k]/v[j-1];
        }
        else {
          maxproduct = maxproduct > v[k] ? maxproduct : v[k];
        }
      }
    }
    std::cout << maxproduct.toString() << std::endl;
  }
