...
void graphCheck(int u){
  ...
  for(auto neighbor : al[u]){
    if(dfs_state[neighbor] == UNVISITED){
      // Normal tree edge
      dfs_parent[neighbor] = u;
      graphCheck(neighbor);
    }
    else if (dfs_state[neighbor] == EXPLORED){
      if( neighbor == dfs_parent[u]){
        // Bidirectional edge
      }
      else{
        // Back Edge
    ...
    else if ( dfs_state[neighbor] == VISITED ){
      // (u, neighbor) is either a foward edge that forms a cycle
      // Or a directed foward edge
    }
  }
  dfs_state[u] = VISITED;
}
